...
[data-testid="stDownloadButton"] button{background:#0d2a43!important;border:1px solid #1c6b9e!important;color:#eaf6ff!important;font-weight:800!important}
[data-testid="stDownloadButton"] button *{color:#eaf6ff!important}
[data-testid="stSelectbox"] div[data-baseweb="select"]>div{background:#09192b!important;color:#eaf6ff!important}
...
def donut(title,data):
    ...
    fig.update_layout(title=dict(text=title,x=.04,font=dict(size=15,color="#f5f8ff")),
        ...
        legend=dict(orientation="h",y=-.08,x=0,font=dict(size=10,color="#d9e9f7")))
    return fig
...
def make_visual_pdf(athlete, cur, sessions, choice):
    ...
    from reportlab.lib import colors
    from reportlab.lib.units import mm
    buf=io.BytesIO(); W,H=landscape(A3); c=canvas.Canvas(buf,pagesize=(W,H))
    bg=colors.HexColor("#06111f"); panel=colors.HexColor("#0b1d31"); white=colors.HexColor("#f5f8ff"); muted=colors.HexColor("#9bb2c8"); blue=colors.HexColor("#1398ff")
    c.setFillColor(bg); c.rect(0,0,W,H,fill=1,stroke=0)
    c.setFillColor(panel); c.roundRect(12*mm,H-34*mm,W-24*mm,22*mm,5*mm,fill=1,stroke=0)
    c.setFillColor(white); c.setFont("Helvetica-Bold",22); c.drawString(20*mm,H-24*mm,"SKATE PERFORMANCE")
    c.setFillColor(muted); c.setFont("Helvetica",9); c.drawString(20*mm,H-29*mm,f"{athlete or 'ATLETA'} | {choice} | {len(sessions)} treino(s)")
    rate=cur["hits"]/cur["attempts"]*100 if cur["attempts"] else 0
    vals=[("TENTATIVAS",cur["attempts"]),("ACERTOS",cur["hits"]),("ERROS",cur["errors"]),("TAXA",f"{rate:.1f}%"),("MANOBRAS",len(cur["maneuvers"]))]
    gap=4*mm; bw=(W-24*mm-gap*4)/5; y=H-65*mm
    for i,(lab,val) in enumerate(vals):
        x=12*mm+i*(bw+gap); c.setFillColor(panel); c.roundRect(x,y,bw,24*mm,4*mm,fill=1,stroke=0); c.setFillColor(muted); c.setFont("Helvetica-Bold",8); c.drawString(x+5*mm,y+16*mm,lab); c.setFillColor(white); c.setFont("Helvetica-Bold",18); c.drawString(x+5*mm,y+6*mm,str(val))
    c.setFillColor(white); c.setFont("Helvetica-Bold",14); c.drawString(12*mm,H-80*mm,"EVOLUCAO ENTRE TREINOS")
    rates=[ss["hits"]/ss["attempts"]*100 if ss["attempts"] else 0 for ss in sessions]; cx=20*mm; cy=H-130*mm; cw=W-40*mm; ch=38*mm
    if rates:
        pts=[]
        for i,v in enumerate(rates): pts.append((cx+cw*(i/max(1,len(rates)-1)),cy+ch*v/100))
        c.setStrokeColor(blue); c.setLineWidth(2)
        for a,b in zip(pts,pts[1:]): c.line(a[0],a[1],b[0],b[1])
        for i,(xx,yy) in enumerate(pts): c.setFillColor(blue); c.circle(xx,yy,2*mm,fill=1,stroke=0); c.setFillColor(white); c.setFont("Helvetica-Bold",7); c.drawCentredString(xx,yy+4*mm,f"{rates[i]:.1f}%")
    c.setFillColor(white); c.setFont("Helvetica-Bold",14); c.drawString(12*mm,H-150*mm,"MANOBRAS")
    yy=H-163*mm
    for m,(hh,ee) in sorted(cur["maneuvers"].items(),key=lambda z:sum(z[1]),reverse=True)[:14]:
        tt=hh+ee; rr=hh/tt*100 if tt else 0; c.setFillColor(panel); c.roundRect(12*mm,yy,W-24*mm,7*mm,1.5*mm,fill=1,stroke=0); c.setFillColor(white); c.setFont("Helvetica",7); c.drawString(16*mm,yy+2.2*mm,f"{m[:45]}    ACERTOS {hh:.0f}    ERROS {ee:.0f}    TOTAL {tt:.0f}    TAXA {rr:.1f}%"); yy-=8*mm
        if yy<10*mm: break
...
with ec1:
    st.download_button("⬇ BAIXAR RELATÓRIO EM PDF",data=pdf_bytes,file_name=f"skate_performance_relatorio_{safe_name}.pdf",mime="application/pdf",use_container_width=True)
with ec2:
    visual_pdf=make_visual_pdf(athlete,cur,sessions,choice)
    st.download_button("⬇ BAIXAR DASHBOARD VISUAL EM PDF",data=visual_pdf,file_name=f"skate_performance_dashboard_{safe_name}.pdf",mime="application/pdf",use_container_width=True)

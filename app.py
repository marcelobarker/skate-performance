...
if len(sessions)>1:
    ...
    yd=[s["cats"]["DIFICULDADE"].get("ALTA",0) for s in sessions]
    figd=go.Figure(go.Scatter(x=x,y=yd,mode="lines+markers+text",
        line=dict(color="#ff4c5b",width=3),marker=dict(size=9,color="#ff4c5b"),
        text=[f"{v:.0f}" for v in yd],textposition="top center"))
    figd.update_layout(height=350,yaxis=dict(title="Manobras / tentativas de dificuldade ALTA",rangemode="tozero",
        gridcolor="#173047"),xaxis=dict(title="Sessão",gridcolor="#173047"),
...
st.download_button(
    ...
    data=pdf_bytes,
    file_name=f"skate_performance_{safe_name}.pdf",

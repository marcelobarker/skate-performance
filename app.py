...
def donut(title,data):
    ...
    if title=="OBSTÁCULO":
        fig.update_traces(domain=dict(x=[0.08,0.92],y=[0.34,1.0]))
        fig.update_layout(title=dict(text=title,x=.04,font=dict(size=15,color="#f5f8ff")),
            height=410,margin=dict(l=8,r=8,t=45,b=145),paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",font=dict(color="#9db3c8"),
            legend=dict(orientation="h",y=-.10,x=0,xanchor="left",yanchor="top",
...
with ec2:
    ...
    #msg{font:11px Arial;color:#8fcfff;margin-top:3px}
    </style>
    <button id="capture">⬇ BAIXAR DASHBOARD COMPLETO EM PDF</button><div id="msg"></div>
    ...
    document.getElementById('capture').onclick=async function(){
      ...
      try{
        ...
        if(ih<=ph){
          pdf.addImage(img,'JPEG',(pw-iw)/2,0,iw,ih,'','FAST');
        }else{
          const pagePx=Math.floor(ph/ratio);
          let y=0,page=0;
          while(y<canvas.height){
            ...
            const simg=slice.toDataURL('image/jpeg',0.94);
            const sh=slice.height*ratio;
            pdf.addImage(simg,'JPEG',(pw-iw)/2,0,iw,sh,'','FAST');
            y+=slice.height; page++;
          }
        }
        pdf.save('skate_performance_dashboard_completo.pdf');
        msg.innerText='PDF gerado.';
      }catch(e){
        msg.innerText='Não foi possível gerar: '+e.message;
      }
    };
    </script>
    """,height=58)
